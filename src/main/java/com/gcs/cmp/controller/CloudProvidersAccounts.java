...
import com.gcs.cmp.service.Cloud_Providers_Accounts_Service;
import com.gcs.cmp.validators.Inputs_Validations;

import io.swagger.v3.oas.annotations.tags.Tag;

@Tag(name = "Providers accounts", description = "Manage cloud providers accounts")
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/cloud/providers/accounts")
public class CloudProvidersAccounts {

	@Autowired
	private Cloud_Providers_Accounts_Service cloud_Accounts_Service;
	...
	@Autowired
	private Providers_Repo providers_Repo;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllCloud_Accounts(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Cloud_Providers_Accounts> result = cloud_Accounts_Service.findCloud_Providers_AccountsList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Cloud_Providers_Accounts> result = cloud_Accounts_Service.findCloud_Providers_AccountsList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addCloud_Accounts(@Validated @RequestBody Cloud_Providers_Accounts cloud_Accounts, Errors errors){
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
			...
			try {
				if(cloud_Accounts.getAccount() == null || cloud_Accounts.getProvider() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					if(cloud_Accounts.getAccount().getId() == 0 || cloud_Accounts.getProvider().getId() == 0) {
				        return ResponseHandler.ResponseBadRequest("Your request was malformed... ID must not be null", HttpStatus.BAD_REQUEST, null);
					}
					Optional<Core_Accounts> account = core_Accounts_Repo.findById(cloud_Accounts.getAccount().getId());
					Optional<Providers> provider = providers_Repo.findById(cloud_Accounts.getProvider().getId());
					if(account.equals(Optional.empty()) || provider.equals(Optional.empty())) {
				        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
					}else {
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(account.get().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								cloud_Accounts.setStart_date(LocalDateTime.now());
								cloud_Accounts.setAccount(account.get());
								cloud_Accounts.setProvider(provider.get());
								Cloud_Providers_Accounts result = cloud_Accounts_Service.addCloudAccount(cloud_Accounts);	
								if(result != null) {
									/*api logs 
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getCloud_AccountsById(@PathVariable(name="id") String id){	
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Cloud_Providers_Accounts> result = cloud_Accounts_Service.findCloudAccountById(Long.parseLong(id));
	...
	public ResponseEntity<Object> findCloudProvidersAccountsByProviderID(@PathVariable(name="provider_id") String provider_id){	
		if(Inputs_Validations.CheckInt(provider_id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				List <Cloud_Providers_Accounts> result = cloud_Accounts_Service.findCloudProvidersAccountsByProviderID(Long.parseLong(provider_id));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}catch(Exception e) {
	            return ResponseHandler.ResponseMulti(e.getMessage(), HttpStatus.MULTI_STATUS, null);
	        }
	...
	public ResponseEntity<Object> updateCloud_Accounts(@RequestBody Cloud_Providers_Accounts cloud_Accounts,@PathVariable(name="id") String id ,Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
		...
		if(Inputs_Validations.CheckInt(id) == false) {
			...
			try {
				Optional<Cloud_Providers_Accounts> cloud_provider_account = cloud_Accounts_Service.findCloudAccountById(Long.parseLong(id));
				if(cloud_provider_account.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else if(cloud_provider_account.get().getStatus() == 4 || cloud_provider_account.get().getEnd_date() != null) {
				...
				}else if(cloud_provider_account.get().getAccount() == null || cloud_provider_account.get().getProvider() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {

					DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"); 

					cloud_Accounts.setAccount(cloud_provider_account.get().getAccount());
					cloud_Accounts.setProvider(cloud_provider_account.get().getProvider());
					
					cloud_Accounts.setStart_date(cloud_provider_account.get().getStart_date());
					
					if(cloud_Accounts.getEnd_date() == null) {
						cloud_Accounts.setEnd_date(cloud_provider_account.get().getEnd_date());
					}
					if(cloud_Accounts.getStatus() == 0) {
						cloud_Accounts.setStatus(cloud_provider_account.get().getStatus());
					}
					if(cloud_Accounts.getCredential_id() == 0) {
						cloud_Accounts.setCredential_id(cloud_provider_account.get().getCredential_id());
					}
					if(cloud_provider_account.get().getAccount() == cloud_Accounts.getAccount() && cloud_provider_account.get().getProvider() == cloud_Accounts.getProvider() && cloud_provider_account.get().getStart_date().toString().equals(cloud_Accounts.getStart_date().format(formatter)) && cloud_Accounts.getEnd_date() == null &&
							cloud_provider_account.get().getStatus() == cloud_Accounts.getStatus() && cloud_provider_account.get().getCredential_id() == cloud_Accounts.getCredential_id()) {
						return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}
					
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(cloud_provider_account.get().getAccount() != null && cloud_provider_account.get().getProvider() != null) {
							if(cloud_provider_account.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								
								cloud_Accounts.setId(Long.parseLong(id));
								Cloud_Providers_Accounts result = cloud_Accounts_Service.updateCloudAccount(cloud_Accounts);
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(cloud_provider_account.get().getAccount() != null && cloud_provider_account.get().getProvider() != null) {
							if(cloud_provider_account.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								cloud_Accounts.setId(Long.parseLong(id));
								Cloud_Providers_Accounts result = cloud_Accounts_Service.updateCloudAccount(cloud_Accounts);
	...
	public ResponseEntity<Object> deleteCloud_Accounts(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				...
				if(cloud_provider_account.equals(Optional.empty())) {
					...
					if(cloud_provider_account.get().getStatus() == 4 || cloud_provider_account.get().getEnd_date() != null) {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(cloud_provider_account.get().getAccount() != null && cloud_provider_account.get().getProvider() != null) {
							if(cloud_provider_account.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								String result = cloud_Accounts_Service.deleteCloudAccount(Long.parseLong(id));
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
