...
import com.gcs.cmp.service.Core_Categories_Service;
import com.gcs.cmp.validators.Inputs_Validations;

import io.swagger.v3.oas.annotations.tags.Tag;

@Tag(name = "Categories", description = "Manage core categoris")
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/core/categories")
public class CoreCategories {

	@Autowired
	private Core_Categories_Service core_Categories_Service;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllCore_Categories(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Core_Categories> result = core_Categories_Service.findCore_CategoriesList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Core_Categories> result = core_Categories_Service.findCore_CategoriesList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addCore_Categories(@Validated @RequestBody Core_Categories core_Categories,Errors errors){
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
					...
					if(account.equals(Optional.empty())) {
						...
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(account.get().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								core_Categories.setAccount(account.get());
								Core_Categories result = core_Categories_Service.addCore_Categories(core_Categories);
								if(result != null) {
									/*api logs 
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getCore_CategoriesById(@PathVariable(name="id") String id){	
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Core_Categories> result = core_Categories_Service.findCore_CategoriesById(Long.parseLong(id));
	...
	public ResponseEntity<Object> updateCore_Categories(@RequestBody Core_Categories core_Categories,@PathVariable(name="id") String id, Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
		...
		if(Inputs_Validations.CheckInt(id) == false) {
			...
			try {
				Optional<Core_Categories> cat = core_Categories_Service.findCore_CategoriesById(Long.parseLong(id));
				if(cat.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else if(cat.get().getStatus() == 4) {
				...
				}else if(cat.get().getAccount() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {					
					core_Categories.setAccount(cat.get().getAccount());
					
					if(core_Categories.getStatus() == 0) {
						core_Categories.setStatus(cat.get().getStatus());
					}
					if(core_Categories.getClasse() == 0) {
						core_Categories.setClasse(cat.get().getClasse());
					}
					if(core_Categories.getName() == null) {
						core_Categories.setName(cat.get().getName());
					}

					if(cat.get().getAccount() == core_Categories.getAccount() && cat.get().getClasse() == core_Categories.getClasse() &&
							cat.get().getStatus() == core_Categories.getStatus() && cat.get().getName().equals(core_Categories.getName())) {
			        	return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}
					
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(cat.get().getAccount() != null) {
							if(cat.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
						        return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Categories.setId(Long.parseLong(id));
								Core_Categories result = core_Categories_Service.updateCore_Categories(core_Categories);
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(cat.get().getAccount() != null) {
							if(cat.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
						        return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Categories.setId(Long.parseLong(id));
								Core_Categories result = core_Categories_Service.updateCore_Categories(core_Categories);
	...
	public ResponseEntity<Object> deleteCore_Categories(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}
		else {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(cat.get().getAccount() != null) {
							if(cat.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								String result = core_Categories_Service.deleteCore_Categories(Long.parseLong(id));
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
