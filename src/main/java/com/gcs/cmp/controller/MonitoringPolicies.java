...
import com.gcs.cmp.service.Monitoring_Policies_Service;
import com.gcs.cmp.validators.Inputs_Validations;

import io.swagger.v3.oas.annotations.tags.Tag;

@Tag(name = "Policies", description = "Manage monitoring policies")
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/monitoring/policies")
public class MonitoringPolicies {

	@Autowired
	private Monitoring_Policies_Service monitoring_Policies_Service;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllMonitoring_Policies(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Monitoring_Policies> result = monitoring_Policies_Service.getMonitoring_PoliciesList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Monitoring_Policies> result = monitoring_Policies_Service.getMonitoring_PoliciesList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addMonitoring_Policies(@Validated @RequestBody Monitoring_Policies monitoring_Policies,Errors errors){
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
					...
					if(account.equals(Optional.empty())) {
						...
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(account.get().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								monitoring_Policies.setStart_date(LocalDateTime.now());
								monitoring_Policies.setAccount(account.get());
								Monitoring_Policies result = monitoring_Policies_Service.addMonitoring_Policies(monitoring_Policies);	
								if(result != null) {
									/*api logs 
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getMonitoring_PoliciesById(@PathVariable(name="id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Monitoring_Policies> result = monitoring_Policies_Service.findMonitoring_PoliciesById(Long.parseLong(id));
	...
	public ResponseEntity<Object> updateMonitoring_Policies(@RequestBody Monitoring_Policies monitoring_Policies,@PathVariable(name="id") String id, Errors errors){
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
		...
		if(Inputs_Validations.CheckInt(id) == false) {
			...
			try {
				Optional<Monitoring_Policies> monitoring_pol = monitoring_Policies_Service.findMonitoring_PoliciesById(Long.parseLong(id));
				if(monitoring_pol.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else if(monitoring_pol.get().getStatus() == 4 || monitoring_pol.get().getEnd_date() != null) {
				...
				}else if(monitoring_pol.get().getAccount() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {

					DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"); 

					monitoring_Policies.setAccount(monitoring_pol.get().getAccount());
					monitoring_Policies.setStart_date(monitoring_pol.get().getStart_date());
					
					if(monitoring_Policies.getEnd_date() == null) {
						monitoring_Policies.setEnd_date(monitoring_pol.get().getEnd_date());
					}
					if(monitoring_Policies.getStatus() == 0) {
						monitoring_Policies.setStatus(monitoring_pol.get().getStatus());
					}
					if(monitoring_Policies.getConditions() == 0) {
						monitoring_Policies.setConditions(monitoring_pol.get().getConditions());
					}
					if(monitoring_Policies.getDuration() == null) {
						monitoring_Policies.setDuration(monitoring_pol.get().getDuration());
					}
					if(monitoring_Policies.getMetric() == 0) {
						monitoring_Policies.setMetric(monitoring_pol.get().getMetric());
					}
					if(monitoring_Policies.getName() == null) {
						monitoring_Policies.setName(monitoring_pol.get().getName());
					}
					if(monitoring_Policies.getThreshold() == null) {
						monitoring_Policies.setThreshold(monitoring_pol.get().getThreshold());
					}
					if(monitoring_pol.get().getDuration() == null) {
						if(monitoring_pol.get().getAccount() == monitoring_Policies.getAccount() && monitoring_pol.get().getStart_date().toString().equals(monitoring_Policies.getStart_date().format(formatter)) && monitoring_Policies.getEnd_date() == null &&
								monitoring_pol.get().getStatus() == monitoring_Policies.getStatus() && monitoring_pol.get().getConditions() == monitoring_Policies.getConditions() && monitoring_pol.get().getDuration() == monitoring_Policies.getDuration() && 
								monitoring_pol.get().getMetric() == monitoring_Policies.getMetric() && monitoring_pol.get().getName().equals(monitoring_Policies.getName()) && monitoring_pol.get().getThreshold().equals(monitoring_Policies.getThreshold())) {
			        	return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}
					}else {
						if(monitoring_pol.get().getAccount() == monitoring_Policies.getAccount() && monitoring_pol.get().getStart_date().toString().equals(monitoring_Policies.getStart_date().format(formatter)) && monitoring_Policies.getEnd_date() == null &&
								monitoring_pol.get().getStatus() == monitoring_Policies.getStatus() && monitoring_pol.get().getConditions() == monitoring_Policies.getConditions() && monitoring_pol.get().getDuration().equals(monitoring_Policies.getDuration()) && 
								monitoring_pol.get().getMetric() == monitoring_Policies.getMetric() && monitoring_pol.get().getName().equals(monitoring_Policies.getName()) && monitoring_pol.get().getThreshold().equals(monitoring_Policies.getThreshold())) {
			        	return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}
					}
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(monitoring_pol.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
				        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
						}else {
							monitoring_Policies.setId(Long.parseLong(id));
							Monitoring_Policies result = monitoring_Policies_Service.updateMonitoring_Policies(monitoring_Policies);
							if(result != null) {
								/*api logs 
								 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
								 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(monitoring_pol.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
				        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
						}else {
							monitoring_Policies.setId(Long.parseLong(id));
							Monitoring_Policies result = monitoring_Policies_Service.updateMonitoring_Policies(monitoring_Policies);
	...
	public ResponseEntity<Object> deleteMonitoring_Policies(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}
		else {
			try {
				...
				if(monitoring_pol.equals(Optional.empty())) {
					...
					if(monitoring_pol.get().getStatus() == 4 || monitoring_pol.get().getEnd_date() != null) {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(monitoring_pol.get().getAccount() != null) {
							if(monitoring_pol.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								String result = monitoring_Policies_Service.deleteMonitoring_Policies(Long.parseLong(id));
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
