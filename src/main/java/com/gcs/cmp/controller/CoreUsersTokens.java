...
import com.gcs.cmp.service.Core_Users_Tokens_Service;
import com.gcs.cmp.validators.Inputs_Validations;

import io.swagger.v3.oas.annotations.tags.Tag;

@Tag(name = "Tokens", description = "Manage users tokens")
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/core/users/tokens")
public class CoreUsersTokens {

	@Autowired
	private Core_Users_Tokens_Service core_Users_Tokens_Service;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllCore_Users_Tokens(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Core_Users_Tokens> result = core_Users_Tokens_Service.getCore_Users_TokensList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Core_Users_Tokens> result = core_Users_Tokens_Service.getCore_Users_TokensList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addCore_Users_Tokens(@Validated @RequestBody Core_Users_Tokens core_Users_Tokens,Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
					...
					if(user.equals(Optional.empty())) {
						...
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(user.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								core_Users_Tokens.setStart_date(LocalDateTime.now());
								core_Users_Tokens.setUser(user.get());
								core_Users_Tokens.setToken(Inputs_Validations.generateKey());
								Core_Users_Tokens result = core_Users_Tokens_Service.addCore_Users_Tokens(core_Users_Tokens);	
								if(result != null) {
									/*api logs 
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getCore_Users_TokensById(@PathVariable(name="id") String id){	
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Core_Users_Tokens> result = core_Users_Tokens_Service.findCore_Users_TokensById(Long.parseLong(id));
	...
	public ResponseEntity<Object> getUsersTokenByUserID(@PathVariable(name="user_id") String user_id){	
		if(Inputs_Validations.CheckInt(user_id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Core_Users_Tokens> result = core_Users_Tokens_Service.getUsersTokenByUserID(Long.parseLong(user_id));
				if(result.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+user_id, HttpStatus.NOT_FOUND, null);
				}else {
		        	return ResponseHandler.ResponseOk("Successfully retrieved data!", HttpStatus.OK, result);
				}
			}catch(Exception e) {
	...
	public ResponseEntity<Object> updateCloud_Accounts(@RequestBody Core_Users_Tokens core_Users_Tokens,@PathVariable(name="id") String id, Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
		...
		if(Inputs_Validations.CheckInt(id) == false) {
			...
			try {
				Optional<Core_Users_Tokens> user_token = core_Users_Tokens_Service.findCore_Users_TokensById(Long.parseLong(id));
				if(user_token.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else if(user_token.get().getStatus() == 4 || user_token.get().getEnd_date() != null) {
				...
				}else if(user_token.get().getUser() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					
					DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"); 

					core_Users_Tokens.setUser(user_token.get().getUser());
					core_Users_Tokens.setStart_date(user_token.get().getStart_date());
					
					if(core_Users_Tokens.getEnd_date() == null) {
						core_Users_Tokens.setEnd_date(user_token.get().getEnd_date());
					}
					if(core_Users_Tokens.getStatus() == 0) {
						core_Users_Tokens.setStatus(user_token.get().getStatus());
					}
					if(core_Users_Tokens.getToken() == null) {
						core_Users_Tokens.setToken(user_token.get().getToken());
					}

					if(user_token.get().getUser() == core_Users_Tokens.getUser() && user_token.get().getStart_date().toString().equals(core_Users_Tokens.getStart_date().format(formatter)) && core_Users_Tokens.getEnd_date() == null &&
							user_token.get().getStatus() == core_Users_Tokens.getStatus() && user_token.get().getToken().equals(core_Users_Tokens.getToken())) {
			        	return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}

					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(user_token.get().getUser().getAccount() != null) {
							if(user_token.get().getUser().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								if(core_Users_Tokens.getToken() == null || core_Users_Tokens.getToken().equals(user_token.get().getToken())) {
									core_Users_Tokens.setToken(user_token.get().getToken());
								}else {
									core_Users_Tokens.setToken(Inputs_Validations.generateKey());
								}
								core_Users_Tokens.setId(Long.parseLong(id));
								Core_Users_Tokens result = core_Users_Tokens_Service.updateCore_Users_Tokens(core_Users_Tokens);
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(user_token.get().getUser().getAccount() != null) {
							if(user_token.get().getUser().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								if(core_Users_Tokens.getToken() == null || core_Users_Tokens.getToken().equals(user_token.get().getToken())) {
									core_Users_Tokens.setToken(user_token.get().getToken());
	...
	public ResponseEntity<Object> deleteCore_Users_Tokens(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}
		else {
			try {
				...
				if(user_token.equals(Optional.empty())) {
					...
					if(user_token.get().getStatus() == 4 || user_token.get().getEnd_date() != null) {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(user_token.get().getUser().getAccount() != null) {
							if(user_token.get().getUser().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								String result = core_Users_Tokens_Service.deleteCore_Users_Tokens(Long.parseLong(id));
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
