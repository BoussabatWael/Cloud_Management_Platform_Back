...
import com.gcs.cmp.service.Core_Users_Metrics_Service;
import com.gcs.cmp.validators.Inputs_Validations;

import io.swagger.v3.oas.annotations.tags.Tag;

@Tag(name = "Users metrics", description = "Manage users metrics")
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/core/users/metrics")
public class CoreUsersMetrics {

	@Autowired
	private Core_Users_Metrics_Service core_Users_Metrics_Service;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllCore_Users_Metrics(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Core_Users_Metrics> result = core_Users_Metrics_Service.getCore_Users_MetricsList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Core_Users_Metrics> result = core_Users_Metrics_Service.getCore_Users_MetricsList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addCore_Users_Metrics(@Validated @RequestBody Core_Users_Metrics core_Users_Metrics,Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
			...
			try {
				if(core_Users_Metrics.getUser() == null || core_Users_Metrics.getMetric() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					if(core_Users_Metrics.getUser().getId() == 0 || core_Users_Metrics.getMetric().getId() == 0) {
				        return ResponseHandler.ResponseBadRequest("Your request was malformed... ID must not be null", HttpStatus.BAD_REQUEST, null);
					}
					Optional<Core_Users> user = core_Users_Repo.findById(core_Users_Metrics.getUser().getId());
					Optional<Metrics> metric = metrics_Repo.findById(core_Users_Metrics.getMetric().getId());
					if(user.equals(Optional.empty()) || metric.equals(Optional.empty())) {
				        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
					}else {
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(user.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								core_Users_Metrics.setStart_date(LocalDateTime.now());
								core_Users_Metrics.setUser(user.get());
								core_Users_Metrics.setMetric(metric.get());
								Core_Users_Metrics result = core_Users_Metrics_Service.addCore_Users_Metrics(core_Users_Metrics);
								if(result != null) {
									/*api logs 
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getCore_Users_MetricsById(@PathVariable(name="id") String id){	
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Core_Users_Metrics> result = core_Users_Metrics_Service.findCore_Users_MetricsById(Long.parseLong(id));
	...
	public ResponseEntity<Object> updateCore_Users_Metrics(@RequestBody Core_Users_Metrics core_Users_Metrics ,@PathVariable(name="id") String id, Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
		...
		if(Inputs_Validations.CheckInt(id) == false) {
			...
			try {
				Optional<Core_Users_Metrics> user_metric = core_Users_Metrics_Service.findCore_Users_MetricsById(Long.parseLong(id));
				if(user_metric.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else if(user_metric.get().getStatus() == 4 || user_metric.get().getEnd_date() != null) {
				...
				}else if(user_metric.get().getUser() == null || user_metric.get().getMetric() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {

					DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"); 

					core_Users_Metrics.setUser(user_metric.get().getUser());
					core_Users_Metrics.setMetric(user_metric.get().getMetric());
					
					core_Users_Metrics.setStart_date(user_metric.get().getStart_date());
					
					if(core_Users_Metrics.getEnd_date() == null) {
						core_Users_Metrics.setEnd_date(user_metric.get().getEnd_date());
					}
					if(core_Users_Metrics.getStatus() == 0) {
						core_Users_Metrics.setStatus(user_metric.get().getStatus());
					}
					
					if(user_metric.get().getUser() == core_Users_Metrics.getUser() && user_metric.get().getMetric() == core_Users_Metrics.getMetric() && user_metric.get().getStart_date().toString().equals(core_Users_Metrics.getStart_date().format(formatter)) && core_Users_Metrics.getEnd_date() == null &&
							user_metric.get().getStatus() == core_Users_Metrics.getStatus()) {
			        	return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}

					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(user_metric.get().getUser().getAccount() != null) {
							if(user_metric.get().getUser().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Users_Metrics.setId(Long.parseLong(id));
								Core_Users_Metrics result = core_Users_Metrics_Service.updateCore_Users_Metrics(core_Users_Metrics);
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(user_metric.get().getUser().getAccount() != null) {
							if(user_metric.get().getUser().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Users_Metrics.setId(Long.parseLong(id));
								Core_Users_Metrics result = core_Users_Metrics_Service.updateCore_Users_Metrics(core_Users_Metrics);
	...
	public ResponseEntity<Object> deleteCore_Users_Metrics(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}
		else {
			try {
				...
				if(user_metric.equals(Optional.empty())) {
					...
					if(user_metric.get().getStatus() == 4 || user_metric.get().getEnd_date() != null) {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(user_metric.get().getUser().getAccount() != null) {
							if(user_metric.get().getUser().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								String result = core_Users_Metrics_Service.deleteCore_Users_Metrics(Long.parseLong(id));
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
