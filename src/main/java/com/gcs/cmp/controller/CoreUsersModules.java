...
import com.gcs.cmp.service.Core_Users_Modules_Service;
import com.gcs.cmp.validators.Inputs_Validations;

import io.swagger.v3.oas.annotations.tags.Tag;

@Tag(name = "Users modules", description = "Manage users modules")
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/core/users/modules")
public class CoreUsersModules {

	@Autowired
	private Core_Users_Modules_Service core_Users_Modules_Service;
	...
	@Autowired
	private Modules_Repo modules_Repo;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllCore_Users_Modules(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Core_Users_Modules> result = core_Users_Modules_Service.getCore_Users_ModulesList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Core_Users_Modules> result = core_Users_Modules_Service.getCore_Users_ModulesList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addCore_Users_Modules(@Validated @RequestBody Core_Users_Modules core_Users_Modules,Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
			...
			try {
				if(core_Users_Modules.getUser() == null || core_Users_Modules.getModule() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					if(core_Users_Modules.getUser().getId() == 0 || core_Users_Modules.getModule().getId() == 0) {
				        return ResponseHandler.ResponseBadRequest("Your request was malformed... ID must not be null", HttpStatus.BAD_REQUEST, null);
					}
					Optional<Core_Users> user = core_Users_Repo.findById(core_Users_Modules.getUser().getId());
					Optional<Modules> module = modules_Repo.findById(core_Users_Modules.getModule().getId());
					if(user.equals(Optional.empty()) || module.equals(Optional.empty())) {
				        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
					}else {
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(user.get().getAccount() != null) {
								if(user.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
									...
									core_Users_Modules.setStart_date(LocalDateTime.now());
									core_Users_Modules.setUser(user.get());
									core_Users_Modules.setModule(module.get());
									Core_Users_Modules result = core_Users_Modules_Service.addCore_Users_Modules(core_Users_Modules);	
									if(result != null) {
										/*api logs 
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getCore_Users_ModulesById(@PathVariable(name="id") String id){	
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Core_Users_Modules> result = core_Users_Modules_Service.findCore_Users_ModulesById(Long.parseLong(id));
	...
	public ResponseEntity<Object> getUsers_ModulesByUserID(@PathVariable(name="user_id") String user_id){	
		if(Inputs_Validations.CheckInt(user_id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						List<Core_Users_Modules> result = core_Users_Modules_Service.getUsers_ModulesByUserID(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT), Long.parseLong(user_id));
						return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
					}else if(BasicAuthInterceptor.GLOBAL_ACCOUNT != null) {
						List<Core_Users_Modules> result = core_Users_Modules_Service.getUsers_ModulesByUserID(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT), Long.parseLong(user_id));
						return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
					}else {
				        return ResponseHandler.ResponseForbidden("Error... Account NOT Identified!", HttpStatus.FORBIDDEN, null);
					}
	...
	public ResponseEntity<Object> updateCore_Users_Modules(@RequestBody Core_Users_Modules core_Users_Modules,@PathVariable(name="id") String id, Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
		...
		if(Inputs_Validations.CheckInt(id) == false) {
			...
			try {
				Optional<Core_Users_Modules> user_module = core_Users_Modules_Service.findCore_Users_ModulesById(Long.parseLong(id));
				if(user_module.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else if(user_module.get().getStatus() == 4 || user_module.get().getEnd_date() != null) {
				...
				}else if(user_module.get().getUser() == null || user_module.get().getModule() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					...
					core_Users_Modules.setUser(user_module.get().getUser());
					core_Users_Modules.setModule(user_module.get().getModule());
					
					core_Users_Modules.setStart_date(user_module.get().getStart_date());
					
					if(core_Users_Modules.getEnd_date() == null) {
						core_Users_Modules.setEnd_date(user_module.get().getEnd_date());
					}
					if(core_Users_Modules.getStatus() == 0) {
						core_Users_Modules.setStatus(user_module.get().getStatus());
					}
					
					if(user_module.get().getUser() == core_Users_Modules.getUser() && user_module.get().getModule() == core_Users_Modules.getModule() && user_module.get().getStart_date().toString().equals(core_Users_Modules.getStart_date().format(formatter)) && core_Users_Modules.getEnd_date() == null &&
							user_module.get().getStatus() == core_Users_Modules.getStatus()) {
			        	return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}

					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(user_module.get().getUser().getAccount() != null) {
							if(user_module.get().getUser().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Users_Modules.setId(Long.parseLong(id));
								Core_Users_Modules result = core_Users_Modules_Service.updateCore_Users_Modules(core_Users_Modules);
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(user_module.get().getUser().getAccount() != null) {
							if(user_module.get().getUser().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Users_Modules.setId(Long.parseLong(id));
								Core_Users_Modules result = core_Users_Modules_Service.updateCore_Users_Modules(core_Users_Modules);
								if(result != null) {
									...
									*/
							        ...
							        return ResponseHandler.ResponseBadRequest("Updated", HttpStatus.OK, result);
								}else {
									return ResponseHandler.ResponseBadRequest("Your request was malformed!", HttpStatus.BAD_REQUEST, null);
								}
							}
	...
	public ResponseEntity<Object> deleteCore_Users_Modules(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}
		else {
			try {
				...
				if(user_module.equals(Optional.empty())) {
					...
					if(user_module.get().getStatus() == 4 || user_module.get().getEnd_date() != null) {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(user_module.get().getUser().getAccount() != null) {
							if(user_module.get().getUser().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								String result = core_Users_Modules_Service.deleteCore_Users_Modules(Long.parseLong(id));
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
