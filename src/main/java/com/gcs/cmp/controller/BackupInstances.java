...
import com.gcs.cmp.service.Backup_Instances_Service;
import com.gcs.cmp.validators.Inputs_Validations;


@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/backup/instances")
public class BackupInstances {

	@Autowired
	private Backup_Instances_Service backup_Instances_Service;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllBackup_Instances(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Backup_Instances> result = backup_Instances_Service.getBackup_InstancesList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Backup_Instances> result = backup_Instances_Service.getBackup_InstancesList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addBackup_Instances(@Validated @RequestBody Backup_Instances backup_Instances,Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
			...
			try {
				if(backup_Instances.getBackup() == null || backup_Instances.getInstance() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					if(backup_Instances.getBackup().getId() == 0 || backup_Instances.getInstance().getId() == 0) {
				        return ResponseHandler.ResponseBadRequest("Your request was malformed... ID must not be null", HttpStatus.BAD_REQUEST, null);
					}
					Optional<Backup_Executions> backup = backup_Executions_Repo.findById(backup_Instances.getBackup().getId());
					Optional<Inventory_Instances> inv_ins = inventory_Instances_Repo.findById(backup_Instances.getInstance().getId());
					if(backup.equals(Optional.empty()) || inv_ins.equals(Optional.empty())) {
				        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
					}else {
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(backup.get().getOperation().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT) || inv_ins.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								backup_Instances.setStart_date(LocalDateTime.now());
								backup_Instances.setBackup(backup.get());
								backup_Instances.setInstance(inv_ins.get());
								Backup_Instances result = backup_Instances_Service.addBackup_Instances(backup_Instances);	
							...
							if(backup.get().getOperation().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT) || inv_ins.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								backup_Instances.setStart_date(LocalDateTime.now());
								backup_Instances.setBackup(backup.get());
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getBackup_InstancesById(@PathVariable(name="id") String id){	
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Backup_Instances> result = backup_Instances_Service.findBackup_InstancesById(Long.parseLong(id));
				if(result.equals(Optional.empty())) {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(result.get().getBackup().getOperation().getAccount() != null && result.get().getInstance().getAccount() != null) {
							if(result.get().getBackup().getOperation().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT) || result.get().getInstance().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
						...
						if(result.get().getBackup().getOperation().getAccount() != null && result.get().getInstance().getAccount() != null) {
							if(result.get().getBackup().getOperation().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT) || result.get().getInstance().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
					        	return ResponseHandler.ResponseOk("Successfully retrieved data!", HttpStatus.OK, result);
							}
	...
	public ResponseEntity<Object> updateBackup_Instances(@RequestBody Backup_Instances backup_Instances,@PathVariable(name="id") String id ,Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
			...
			try {
				Optional<Backup_Instances> b_ins = backup_Instances_Service.findBackup_InstancesById(Long.parseLong(id));
				if(b_ins.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found with id "+id, HttpStatus.NOT_FOUND, null);
				}else if(b_ins.get().getStatus() == 4 || b_ins.get().getEnd_date() != null) {
				...
				}else if(b_ins.get().getBackup() == null || b_ins.get().getInstance() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					
					DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"); 

					backup_Instances.setBackup(b_ins.get().getBackup());
					backup_Instances.setInstance(b_ins.get().getInstance());
					backup_Instances.setStart_date(b_ins.get().getStart_date());
					
					if(backup_Instances.getEnd_date() == null) {
						backup_Instances.setEnd_date(b_ins.get().getEnd_date());
					
						if(backup_Instances.getStatus() == 0) {
							backup_Instances.setStatus(b_ins.get().getStatus());
						}
						
						if(b_ins.get().getBackup() == backup_Instances.getBackup() && b_ins.get().getInstance() == backup_Instances.getInstance() && b_ins.get().getStart_date().toString().equals(backup_Instances.getStart_date().format(formatter)) && backup_Instances.getEnd_date() == null &&
								b_ins.get().getStatus() == backup_Instances.getStatus()) {
							return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
						}
					}

					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(b_ins.get().getBackup().getOperation().getAccount() != null && b_ins.get().getInstance().getAccount() != null ) {
							if(b_ins.get().getBackup().getOperation().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT) || b_ins.get().getInstance().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								backup_Instances_Service.findBackup_InstancesById(Long.parseLong(id)).get();
								backup_Instances.setId(Long.parseLong(id));
								Backup_Instances result = backup_Instances_Service.updateBackup_Instances(backup_Instances);
								if(result != null) {
									// create logs 
									// backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									//core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(b_ins.get().getBackup().getOperation().getAccount() != null && b_ins.get().getInstance().getAccount() != null ) {
							if(b_ins.get().getBackup().getOperation().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT) || b_ins.get().getInstance().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								backup_Instances_Service.findBackup_InstancesById(Long.parseLong(id)).get();
								backup_Instances.setId(Long.parseLong(id));
	...
	public ResponseEntity<Object> deleteBackup_Instances(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				...
				if(b_ins.equals(Optional.empty())) {
					...
					if(b_ins.get().getStatus() == 4 || b_ins.get().getEnd_date() != null) {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(b_ins.get().getBackup().getOperation().getAccount() != null && b_ins.get().getInstance().getAccount() != null ) {
							if(b_ins.get().getBackup().getOperation().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT) || b_ins.get().getInstance().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								String result = backup_Instances_Service.deleteBackup_Instances(Long.parseLong(id));
								if(result != null) {
									// create logs 
									// backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									//core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
