...
import com.gcs.cmp.service.Core_Categories_Elements_Service;
import com.gcs.cmp.validators.Inputs_Validations;


@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/core/categories/elements")
public class CoreCategoriesElements {

	@Autowired
	private Core_Categories_Elements_Service core_Categories_Elements_Service;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllCore_Categories_Elements(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Core_Categories_Elements> result = core_Categories_Elements_Service.findCore_Categories_ElementsList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Core_Categories_Elements> result = core_Categories_Elements_Service.findCore_Categories_ElementsList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addCore_Categories_Elements(@Validated @RequestBody Core_Categories_Elements core_Categories_Elements,Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		...
		if(errors.hasErrors()) {
			...
			try {
				if(core_Categories_Elements.getCategory() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					if(core_Categories_Elements.getCategory().getId() == 0) {
				        return ResponseHandler.ResponseBadRequest("Your request was malformed... ID must not be null", HttpStatus.BAD_REQUEST, null);
					}
					Optional<Core_Categories> category = core_Categories_Repo.findById(core_Categories_Elements.getCategory().getId());
					if(category.equals(Optional.empty())) {
				        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
					}else {
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(category.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Categories_Elements.setCategory(category.get());
								
								Core_Categories_Elements result = core_Categories_Elements_Service.addCore_Categories_Elements(core_Categories_Elements);
								if(result != null) {
									// create logs 
							...
							if(category.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Categories_Elements.setCategory(category.get());
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getCore_Categories_ElementsById(@PathVariable(name="id") String id){	
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Core_Categories_Elements> result = core_Categories_Elements_Service.findCore_Categories_ElementsById(Long.parseLong(id));
				if(result.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else {
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(result.get().getCategory().getAccount() != null) {
							if(result.get().getCategory().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
						...
						if(result.get().getCategory().getAccount() != null) {
							if(result.get().getCategory().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
						        return ResponseHandler.ResponseOk("Successfully retrieved data!", HttpStatus.OK, result);
							}
	...
	public ResponseEntity<Object> updateCore_Categories_Elements(@RequestBody Core_Categories_Elements core_Categories_Elements,@PathVariable(name="id") String id, Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
		...
		if(Inputs_Validations.CheckInt(id) == false) {
			...
			try {
				Optional<Core_Categories_Elements> cat_elem = core_Categories_Elements_Service.findCore_Categories_ElementsById(Long.parseLong(id));
				if(cat_elem.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else if(cat_elem.get().getCategory() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					core_Categories_Elements.setCategory(cat_elem.get().getCategory());

					if(core_Categories_Elements.getElement() == 0) {
						core_Categories_Elements.setElement(cat_elem.get().getElement());
					}
					if(core_Categories_Elements.getElement_id() == 0) {
						core_Categories_Elements.setElement_id(cat_elem.get().getElement_id());
					}
					
					if(cat_elem.get().getCategory() == core_Categories_Elements.getCategory() && cat_elem.get().getElement() == core_Categories_Elements.getElement() && cat_elem.get().getElement_id() == core_Categories_Elements.getElement_id()) {
			        	return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}
					
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(cat_elem.get().getCategory().getAccount() != null) {
							if(cat_elem.get().getCategory().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Categories_Elements.setId(Long.parseLong(id));
								Core_Categories_Elements result =core_Categories_Elements_Service.updateCore_Categories_Elements(core_Categories_Elements);
								if(result !=  null) {
									// create logs 
									// backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									//core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(cat_elem.get().getCategory().getAccount() != null) {
							if(cat_elem.get().getCategory().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Categories_Elements.setId(Long.parseLong(id));
								Core_Categories_Elements result =core_Categories_Elements_Service.updateCore_Categories_Elements(core_Categories_Elements);
	...
	public ResponseEntity<Object> deleteCore_Categories_Elements(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}
		else {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(cat_elem.get().getCategory().getAccount() != null) {
							if(cat_elem.get().getCategory().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								String result = core_Categories_Elements_Service.deleteCore_Categories_Elements(Long.parseLong(id));
								if(result != null) {
									// create logs 
									// backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									//core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
