...
import com.gcs.cmp.service.Core_Accounts_Modules_Service;
import com.gcs.cmp.validators.Inputs_Validations;

import io.swagger.v3.oas.annotations.tags.Tag;

@Tag(name = "Accounts modules", description = "Manage core accounts modules")
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/core/accounts/modules")
public class CoreAccountsModules {

	@Autowired
	private Core_Accounts_Modules_Service core_Accounts_Modules_Service;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllCore_Accounts_Modules(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Core_Accounts_Modules> result = core_Accounts_Modules_Service.findCore_Accounts_ModulesList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Core_Accounts_Modules> result = core_Accounts_Modules_Service.findCore_Accounts_ModulesList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addCore_Accounts_Modules(@Validated @RequestBody Core_Accounts_Modules core_Accounts_Modules,Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
			...
			try {
				if(core_Accounts_Modules.getAccount() == null || core_Accounts_Modules.getModule() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					if(core_Accounts_Modules.getAccount().getId() == 0 || core_Accounts_Modules.getModule().getId() == 0) {
				        return ResponseHandler.ResponseBadRequest("Your request was malformed... ID must not be null", HttpStatus.BAD_REQUEST, null);
					}
					Optional<Core_Accounts> account = core_Accounts_Repo.findById(core_Accounts_Modules.getAccount().getId());
					Optional<Modules> module = modules_Repo.findById(core_Accounts_Modules.getModule().getId());
					if(account.equals(Optional.empty()) || module.equals(Optional.empty())) {
				        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
					}else {
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(account.get().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								core_Accounts_Modules.setStart_date(LocalDateTime.now());
								core_Accounts_Modules.setAccount(account.get());
								core_Accounts_Modules.setModule(module.get());
								Core_Accounts_Modules result = core_Accounts_Modules_Service.addCore_Accounts_Modules(core_Accounts_Modules);	
								if(result != null) {
									/*api logs 
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getCore_Accounts_ModulesById(@PathVariable(name="id") String id){	
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Core_Accounts_Modules> result = core_Accounts_Modules_Service.findCore_Accounts_ModulesById(Long.parseLong(id));
				if(result.equals(Optional.empty())) {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(result.get().getAccount() != null && result.get().getModule() != null) {
	...
	public ResponseEntity<Object> updateCore_Accounts_Modules(@RequestBody Core_Accounts_Modules core_Accounts_Modules,@PathVariable(name="id") String id, Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
		...
		if(Inputs_Validations.CheckInt(id) == false) {
			...
			try {
				Optional<Core_Accounts_Modules> account_module = core_Accounts_Modules_Service.findCore_Accounts_ModulesById(Long.parseLong(id));
				if(account_module.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else if(account_module.get().getStatus() == 4 || account_module.get().getEnd_date() != null) {
				...
				}else if(account_module.get().getAccount() == null || account_module.get().getModule() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"); 
					
					core_Accounts_Modules.setAccount(account_module.get().getAccount());
					core_Accounts_Modules.setModule(account_module.get().getModule());
					core_Accounts_Modules.setStart_date(account_module.get().getStart_date());
					
					if(core_Accounts_Modules.getEnd_date() == null) {
						core_Accounts_Modules.setEnd_date(account_module.get().getEnd_date());
					}
					if(core_Accounts_Modules.getStatus() == 0) {
						core_Accounts_Modules.setStatus(account_module.get().getStatus());
					}

					if(account_module.get().getAccount() == core_Accounts_Modules.getAccount() && account_module.get().getModule() == core_Accounts_Modules.getModule() && account_module.get().getStart_date().toString().equals(core_Accounts_Modules.getStart_date().format(formatter)) && core_Accounts_Modules.getEnd_date() == null &&
							account_module.get().getStatus() == core_Accounts_Modules.getStatus()) {
			        	return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}
					
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(account_module.get().getAccount() != null) {
							if(account_module.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Accounts_Modules.setId(Long.parseLong(id));
								Core_Accounts_Modules result = core_Accounts_Modules_Service.updateCore_Accounts_Modules(core_Accounts_Modules);
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(account_module.get().getAccount() != null) {
							if(account_module.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Accounts_Modules.setId(Long.parseLong(id));
								Core_Accounts_Modules result = core_Accounts_Modules_Service.updateCore_Accounts_Modules(core_Accounts_Modules);
	...
	public ResponseEntity<Object> deleteCore_Accounts_Modules(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}
		else {
			try {
				...
				if(account_module.equals(Optional.empty())) {
					...
					if(account_module.get().getStatus() == 4 || account_module.get().getEnd_date() != null) {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(account_module.get().getAccount() != null || account_module.get().getModule() != null) {
							if(account_module.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								String result = core_Accounts_Modules_Service.deleteCore_Accounts_Modules(Long.parseLong(id));
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
					...
					}else if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(account_module.get().getAccount() != null || account_module.get().getModule() != null) {
							if(account_module.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
