...
import com.gcs.cmp.service.Core_Users_Instances_Service;
import com.gcs.cmp.validators.Inputs_Validations;

import io.swagger.v3.oas.annotations.tags.Tag;

@Tag(name = "Users instances", description = "Manage users instances")
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/core/users/instances")
public class CoreUsersInstances {

	@Autowired
	private Core_Users_Instances_Service core_Users_Instances_Service;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllCore_Users_Instances(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Core_Users_Instances> result = core_Users_Instances_Service.getCore_Users_InstancesList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Core_Users_Instances> result = core_Users_Instances_Service.getCore_Users_InstancesList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addCore_Users_Instances(@Validated @RequestBody Core_Users_Instances core_Users_Instances,Errors errors){
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
					...
					if(user.equals(Optional.empty())) {
						...
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(user.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								core_Users_Instances.setStart_date(LocalDateTime.now());
								core_Users_Instances.setUser(user.get());
								Core_Users_Instances result = core_Users_Instances_Service.addCore_Users_Instances(core_Users_Instances);	
								if(result != null) {
									/*api logs 
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getCore_Users_InstancesById(@PathVariable(name="id") String id){	
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Core_Users_Instances> result = core_Users_Instances_Service.findCore_Users_InstancesById(Long.parseLong(id));
	...
	public ResponseEntity<Object> getUsers_InstancesByUserID(@PathVariable(name="user_id") String user_id){	
		if(Inputs_Validations.CheckInt(user_id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						List<Core_Users_Instances> result = core_Users_Instances_Service.getUsers_InstancesByUserID(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT), Long.parseLong(user_id));
						return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);

					}else if(BasicAuthInterceptor.GLOBAL_ACCOUNT != null) {
						List<Core_Users_Instances> result = core_Users_Instances_Service.getUsers_InstancesByUserID(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT), Long.parseLong(user_id));
						return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
					}else {
				        return ResponseHandler.ResponseForbidden("Error... Account NOT Identified!", HttpStatus.FORBIDDEN, null);
					}
	...
	public ResponseEntity<Object> updateCore_Users_Instances(@RequestBody Core_Users_Instances core_Users_Instances,@PathVariable(name="id") String id, Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
		...
		if(Inputs_Validations.CheckInt(id) == false) {
			...
			try {
				Optional<Core_Users_Instances> user_ins = core_Users_Instances_Service.findCore_Users_InstancesById(Long.parseLong(id));
				if(user_ins.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else if(user_ins.get().getStatus() == 4 || user_ins.get().getEnd_date() != null) {
				...
				}else if(user_ins.get().getUser() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"); 
					
					core_Users_Instances.setUser(user_ins.get().getUser());
					core_Users_Instances.setStart_date(user_ins.get().getStart_date());
					
					if(core_Users_Instances.getEnd_date() == null) {
						core_Users_Instances.setEnd_date(user_ins.get().getEnd_date());
					}
					if(core_Users_Instances.getStatus() == 0) {
						core_Users_Instances.setStatus(user_ins.get().getStatus());
					}
					if(core_Users_Instances.getElement() == 0) {
						core_Users_Instances.setElement(user_ins.get().getElement());
					}
					if(core_Users_Instances.getElement_id() == 0) {
						core_Users_Instances.setElement_id(user_ins.get().getElement_id());
					}

					if(user_ins.get().getUser() == core_Users_Instances.getUser() && user_ins.get().getStart_date().toString().equals(core_Users_Instances.getStart_date().format(formatter)) && core_Users_Instances.getEnd_date() == null &&
							user_ins.get().getStatus() == core_Users_Instances.getStatus() && user_ins.get().getElement() == core_Users_Instances.getElement() && user_ins.get().getElement_id() == core_Users_Instances.getElement_id()) {
			        	return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}

					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(user_ins.get().getUser().getAccount() != null){
							if(user_ins.get().getUser().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Users_Instances.setId(Long.parseLong(id));
								Core_Users_Instances result = core_Users_Instances_Service.updateCore_Users_Instances(core_Users_Instances);
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(user_ins.get().getUser().getAccount() != null){
							if(user_ins.get().getUser().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								core_Users_Instances.setId(Long.parseLong(id));
								Core_Users_Instances result = core_Users_Instances_Service.updateCore_Users_Instances(core_Users_Instances);
	...
	public ResponseEntity<Object> deleteCore_Users_Instances(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				...
				if(user_ins.equals(Optional.empty())) {
					...
					if(user_ins.get().getStatus() == 4 || user_ins.get().getEnd_date() != null) {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(user_ins.get().getUser().getAccount() != null) {
							if(user_ins.get().getUser().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								String result = core_Users_Instances_Service.deleteCore_Users_Instances(Long.parseLong(id));
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
