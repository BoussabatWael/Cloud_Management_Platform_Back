...
import com.gcs.cmp.service.Monitoring_Settings_Service;
import com.gcs.cmp.validators.Inputs_Validations;

import io.swagger.v3.oas.annotations.tags.Tag;

@Tag(name = "Monitoring settings", description = "Manage monitoring settings")
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/monitoring/settings")
public class MonitoringSettings {

	@Autowired
	private Monitoring_Settings_Service monitoring_Settings_Service;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllMonitoring_Settings(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Monitoring_Settings> result = monitoring_Settings_Service.getMonitoring_SettingsList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Monitoring_Settings> result = monitoring_Settings_Service.getMonitoring_SettingsList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addMonitoring_Settings(@Validated @RequestBody Monitoring_Settings monitoring_Settings,Errors errors){
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
					...
					if(account.equals(Optional.empty())) {
						...
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(account.get().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								monitoring_Settings.setAccount(account.get());
								Monitoring_Settings result = monitoring_Settings_Service.addMonitoring_Settings(monitoring_Settings);	
								if(result != null) {
									/*api logs 
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getMonitoring_SettingsById(@PathVariable(name="id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Monitoring_Settings> result = monitoring_Settings_Service.findMonitoring_SettingsById(Long.parseLong(id));
	...
	public ResponseEntity<Object> updateMonitoring_Settings(@RequestBody Monitoring_Settings monitoring_Settings,@PathVariable(name="id") String id, Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
		...
		if(Inputs_Validations.CheckInt(id) == false) {
			...
			try {
				Optional<Monitoring_Settings> monitoring_setting = monitoring_Settings_Service.findMonitoring_SettingsById(Long.parseLong(id));
				if(monitoring_setting.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else if(monitoring_setting.get().getAccount() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {

					monitoring_Settings.setAccount(monitoring_setting.get().getAccount());
					
					if(monitoring_Settings.getAgent_interval() == 0) {
						monitoring_Settings.setAgent_interval(monitoring_setting.get().getAgent_interval());
					}
					if(monitoring_Settings.getAgent_severity() == 0) {
						monitoring_Settings.setAgent_severity(monitoring_setting.get().getAgent_severity());
					}
					if(monitoring_Settings.getAgent_status() == 0) {
						monitoring_Settings.setAgent_status(monitoring_setting.get().getAgent_status());
					}

					if(monitoring_setting.get().getAccount() == monitoring_Settings.getAccount() && monitoring_setting.get().getAgent_interval() == monitoring_Settings.getAgent_interval() &&
							monitoring_setting.get().getAgent_severity() == monitoring_Settings.getAgent_severity() && monitoring_setting.get().getAgent_status() == monitoring_Settings.getAgent_status()) {
			        	return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}

					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(monitoring_setting.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
				        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
						}else {
							monitoring_Settings.setId(Long.parseLong(id));
							Monitoring_Settings result = monitoring_Settings_Service.updateMonitoring_Settings(monitoring_Settings);
							if(result != null) {
								/*api logs 
								 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
								 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(monitoring_setting.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
				        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
						}else {
							monitoring_Settings.setId(Long.parseLong(id));
							Monitoring_Settings result = monitoring_Settings_Service.updateMonitoring_Settings(monitoring_Settings);
	...
	public ResponseEntity<Object> deleteMonitoring_Settings(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}
		else {
			try {
				...
				if(monitoring_setting.equals(Optional.empty())) {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(monitoring_setting.get().getAccount() != null) {
							if(monitoring_setting.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								String result = monitoring_Settings_Service.deleteMonitoring_Settings(Long.parseLong(id));
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
				        ...
				        return ResponseHandler.ResponseBadRequest("Error... Account NOT Identified!", HttpStatus.FORBIDDEN, null);
					}
				}
			}catch (Exception e) {
	            return ResponseHandler.ResponseMulti(e.getMessage(), HttpStatus.MULTI_STATUS, null);
