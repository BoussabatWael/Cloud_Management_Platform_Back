...
import com.gcs.cmp.service.Monitoring_Automations_Service;
import com.gcs.cmp.validators.Inputs_Validations;

import io.swagger.v3.oas.annotations.tags.Tag;

@Tag(name = "Automations", description = "Manage monitoring automations")
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/monitoring/automations")
public class MonitoringAutomations {

	@Autowired
	private Monitoring_Automations_Service monitoring_Automations_Service;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllMonitoring_Automations(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Monitoring_Automations> result = monitoring_Automations_Service.getMonitoring_AutomationsList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Monitoring_Automations> result = monitoring_Automations_Service.getMonitoring_AutomationsList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addMonitoring_Automations(@Validated @RequestBody Monitoring_Automations monitoring_Automations,Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
					...
					if(account.equals(Optional.empty())) {
						...
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(account.get().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								monitoring_Automations.setStart_date(LocalDateTime.now());
								monitoring_Automations.setAccount(account.get());
								Monitoring_Automations result = monitoring_Automations_Service.addMonitoring_Automations(monitoring_Automations);	
								if(result != null) {
									/*api logs 
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getMonitoring_AutomationsById(@PathVariable(name="id") String id){	
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Monitoring_Automations> result = monitoring_Automations_Service.findMonitoring_AutomationsById(Long.parseLong(id));
	...
	public ResponseEntity<Object> updateMonitoring_Automations(@RequestBody Monitoring_Automations monitoring_Automations,@PathVariable(name="id") String id, Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
		...
		if(Inputs_Validations.CheckInt(id) == false) {
			...
			try {
				Optional<Monitoring_Automations> montoring_automation = monitoring_Automations_Service.findMonitoring_AutomationsById(Long.parseLong(id));
				if(montoring_automation.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else if(montoring_automation.get().getStatus() == 4 || montoring_automation.get().getEnd_date() != null) {
				...
				}else if(montoring_automation.get().getAccount() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {

					DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"); 

					monitoring_Automations.setAccount(montoring_automation.get().getAccount());
					monitoring_Automations.setStart_date(montoring_automation.get().getStart_date());
					
					if(monitoring_Automations.getEnd_date() == null) {
						monitoring_Automations.setEnd_date(montoring_automation.get().getEnd_date());
					}
					if(monitoring_Automations.getStatus() == 0) {
						monitoring_Automations.setStatus(montoring_automation.get().getStatus());
					}
					if(monitoring_Automations.getClasse() == 0) {
						monitoring_Automations.setClasse(montoring_automation.get().getClasse());
					}
					if(monitoring_Automations.getElement() == 0) {
						monitoring_Automations.setElement(montoring_automation.get().getElement());
					}
					if(monitoring_Automations.getElement_id() == 0) {
						monitoring_Automations.setElement_id(montoring_automation.get().getElement_id());
					}
					if(monitoring_Automations.getScale() == 0) {
						monitoring_Automations.setClasse(montoring_automation.get().getScale());
					}
					if(monitoring_Automations.getName() == null) {
						monitoring_Automations.setName(montoring_automation.get().getName());
					}
					if(monitoring_Automations.getUnit() == null) {
						monitoring_Automations.setUnit(montoring_automation.get().getUnit());
					}
					
					if(montoring_automation.get().getAccount() == monitoring_Automations.getAccount() && montoring_automation.get().getStart_date().toString().equals(monitoring_Automations.getStart_date().format(formatter)) && monitoring_Automations.getEnd_date() == null &&
							montoring_automation.get().getStatus() == monitoring_Automations.getStatus() && montoring_automation.get().getClasse() == monitoring_Automations.getClasse() && montoring_automation.get().getElement() == monitoring_Automations.getElement() && 
							montoring_automation.get().getElement_id() == monitoring_Automations.getElement_id() && montoring_automation.get().getScale() == monitoring_Automations.getScale() && montoring_automation.get().getName().equals(monitoring_Automations.getName())
							 && montoring_automation.get().getUnit().equals(monitoring_Automations.getUnit())) {
			        	return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}
					
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(montoring_automation.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
				        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
						}else {
							monitoring_Automations.setId(Long.parseLong(id));
							Monitoring_Automations result = monitoring_Automations_Service.updateMonitoring_Automations(monitoring_Automations);
							if(result != null) {
								/*api logs 
								 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
								 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(montoring_automation.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
				        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
						}else {
							monitoring_Automations.setId(Long.parseLong(id));
							Monitoring_Automations result = monitoring_Automations_Service.updateMonitoring_Automations(monitoring_Automations);
	...
	public ResponseEntity<Object> deleteMonitoring_Automations(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}
		else {
			try {
				...
				if(montoring_automation.equals(Optional.empty())) {
					...
					if(montoring_automation.get().getStatus() == 4 || montoring_automation.get().getEnd_date() != null) {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(montoring_automation.get().getAccount() != null) {
							if(montoring_automation.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								String result = monitoring_Automations_Service.deleteMonitoring_Automations(Long.parseLong(id));
								if(result != null) {
									/*api logs 
									 * backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									 * core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
