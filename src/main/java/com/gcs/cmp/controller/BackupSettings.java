...
import com.gcs.cmp.service.Backup_Settings_Service;
import com.gcs.cmp.validators.Inputs_Validations;


@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/backup/settings")
public class BackupSettings {

	@Autowired
	private Backup_Settings_Service backup_Settings_Service;
	...
	@GetMapping("")
	public ResponseEntity<Object> getAllBackup_Settings(){	
		try {
			if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
				List <Backup_Settings> result = backup_Settings_Service.findBackup_SettingsList(Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT));	
				...
				List <Backup_Settings> result = backup_Settings_Service.findBackup_SettingsList(Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT));	
				return ResponseHandler.ResponseListOk("Successfully retrieved data!", result.size(), HttpStatus.OK, result);
			}else {
	...
	@PostMapping("/create")
	public ResponseEntity<Object> addBackup_Settings(@Validated @RequestBody Backup_Settings backup_Settings,Errors errors){
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
					...
					if(account.equals(Optional.empty())) {
						...
						if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
							if(account.get().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								backup_Settings.setAccount(account.get());
								Backup_Settings result = backup_Settings_Service.addBackup_Settings(backup_Settings);	
								if(result != null) {
									// create logs 
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<Object> getBackup_SettingsById(@PathVariable(name="id") String id){	
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
				Optional<Backup_Settings> result = backup_Settings_Service.findBackup_SettingsById(Long.parseLong(id));
	...
	public ResponseEntity<Object> updateBackup_Settings(@RequestBody Backup_Settings backup_Settings,@PathVariable(name="id") String id ,Errors errors){	
		if(ex != null) {
			handleException(ex);
		}
		if(errors.hasErrors()) {
		...
		if(Inputs_Validations.CheckInt(id) == false) {
			...
			try {
				Optional<Backup_Settings> b_set = backup_Settings_Service.findBackup_SettingsById(Long.parseLong(id));
				if(b_set.equals(Optional.empty())) {
		        	return ResponseHandler.ResponseNotFound("No results were found for id "+id, HttpStatus.NOT_FOUND, null);
				}else if(b_set.get().getAccount() == null) {
			        return ResponseHandler.ResponseBadRequest("Your request was malformed! Foreign key required!", HttpStatus.BAD_REQUEST, null);
				}else {
					backup_Settings.setAccount(b_set.get().getAccount());
					
					
					if(backup_Settings.getBackup_retention_count() == 0) {
						backup_Settings.setBackup_retention_count(b_set.get().getBackup_retention_count());
					}
					if(backup_Settings.getBackup_storage() == 0) {
						backup_Settings.setBackup_storage(b_set.get().getBackup_storage());
					}
					if(backup_Settings.getDefault_backup_schedule() == 0) {
						backup_Settings.setDefault_backup_schedule(b_set.get().getDefault_backup_schedule());
					}
					if(backup_Settings.getScheduled_backups() == 0) {
						backup_Settings.setScheduled_backups(b_set.get().getScheduled_backups());
					}
					
					if(b_set.get().getAccount() == backup_Settings.getAccount() && b_set.get().getBackup_retention_count() == backup_Settings.getBackup_retention_count() && b_set.get().getBackup_storage() == backup_Settings.getBackup_storage() &&
							b_set.get().getDefault_backup_schedule() == backup_Settings.getDefault_backup_schedule() && b_set.get().getScheduled_backups() == backup_Settings.getScheduled_backups()) {
						return ResponseHandler.ResponseOk("Nothing was changed!", HttpStatus.OK, null);
					}
					
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(b_set.get().getAccount() != null) {
							if(b_set.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								backup_Settings.setId(Long.parseLong(id));
								Backup_Settings result = backup_Settings_Service.updateBackup_Settings(backup_Settings);
								if(result != null) {
									// create logs 
									// backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									//core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
						...
						if(b_set.get().getAccount() != null) {
							if(b_set.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_ACCOUNT)) {
					        	return ResponseHandler.ResponseForbidden("You are not allowed to perform that action!", HttpStatus.FORBIDDEN, null);
							}else {
								backup_Settings.setId(Long.parseLong(id));
								Backup_Settings result = backup_Settings_Service.updateBackup_Settings(backup_Settings);
	...
	public ResponseEntity<Object> deleteBackup_Settings(@PathVariable("id") String id){
		if(Inputs_Validations.CheckInt(id) == false) {
            return ResponseHandler.ResponseMulti("PATH malformed!", HttpStatus.MULTI_STATUS, null);
		}else {
			try {
					...
					if(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT != null) {
						if(b_set.get().getAccount() != null) {
							if(b_set.get().getAccount().getId() != Long.parseLong(BasicAuthInterceptor.GLOBAL_USER_ACCOUNT)) {
								...
								String result = backup_Settings_Service.deleteBackup_Settings(Long.parseLong(id));
								if(result != null) {
									// create logs 
									// backup_executions:1,backup_instances:2,backup_operations:3,backup_settings:4,cloud_providers_account:5,core_access_credentials:6,core_accounts_modules:7,core_accounts:8,core_categories_elements:9,core_categories:10,
									//core_logs:11,core_notes:12,core_notifications:13,core_urls:14,core_users_instances:15,core_users_metrics:16,core_users_modules:17,core_users_permissions:18,core_users_security:19,core_users_tokens:20,core_users:21,
